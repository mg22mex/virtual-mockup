...
BACKPACK_FRONT_OVERLAYS: dict[str, dict[str, dict[str, Any]]] = {
    "upper_center": {
        ...
        "black": {
            "box": (595.0, 833.0, 198.0, 56.0),
            "cover": (420.0, 750.0, 400.0, 260.0),
            "erase": "photo",
            "rotate": -7.0,
        },
    },
    "center": {
        ...
        "black": {
            "box": (595.0, 1020.0, 198.0, 56.0),
            "cover": (420.0, 920.0, 400.0, 280.0),
            "erase": "photo",
            "rotate": -7.0,
        },
    },
...
def resolve_backpack_v2_page_index(
    ...
    Upper center → Options #1–#3, Center → #4–#6, Lower right center → #7–#9.
    Fabric selects the matching colorway page so Front View / swatches match.
    """
    from utils.catalog import resolve_backpack_placement
...
def tint_with_alpha_mask(
    ...
    *,
    ...
    try:
        ...
        # Preserve dark shadows and hardware lines while tinting.
        multiply_layer = ImageChops.multiply(roi, solid)
        multiply_layer = _match_layer(multiply_layer, size, "RGB")
        composited = image_composite(multiply_layer, roi, mask_roi)
