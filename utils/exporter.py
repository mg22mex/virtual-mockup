...
class WorksheetExporter:
    ...
    def render_backpack_with_pdf_template(
        ...
        """Venture Dry Pack — Paula v2 page with Front / Artwork / GS panels.

        Selects Options #1–#9 by placement × fabric for the full layout
        (Front View photo, Artwork box, flat tech-pack + swatches). Recolors
        the uploaded mark to ``Logo / graphic color`` (or keeps native art for
        ``Match uploaded art``) and stamps Artwork + Front + Graphic Sample.
        ...
        # Header + project chip (Paula coords).
        self._pdf_overlay_header(page, spec, job)
        self._pdf_overlay_chip(page, spec, job)

        # Fabric / Logo color swatch labels on the Design Placement key.
        self._pdf_overlay_colors(page, spec, job)

        if mark is not None:
            # Artwork box (contrast canvas + recolored client mark).
            art_slot = next(
            ...
            # Flat tech-pack Graphic Sample (Option #1 / #4 / #7 placement).
